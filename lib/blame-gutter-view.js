...
class BlameGutterView {
  ...
  diffClicked(hash) {
    getDiff(this.editor.getPath(), hash.replace(/^[\^]/, ''), (diff) => {
      if (diff) {
        atom.workspace.open()
          .then(editor => {
            editor.setText(diff)
            editor.setGrammar(atom.grammars.grammarForScopeName('source.diff'))
          })
      }
    })
  }
  ...
  addMarker(lineNo, hash, rowCls, lineStr) {
    ...
    if (lineStr.length > 0) {
      if (this.isCommited(hash)) {
        item.appendChild(this.diffSpan(hash))
  ...
  diffSpan(hash) {
    return this.iconSpan(hash, 'diff', () => {
      this.diffClicked(hash)
    })
  }
