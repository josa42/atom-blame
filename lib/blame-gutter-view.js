...
class BlameGutterView {
  ...
  update() {
    var args = atom.config.get('blame.commandLineArgs')
    blame(this.editor.getPath(), args, (result) => {
      if (!this.visible) {
        return
      }
  ...
  addMarker(lineNo, hash, rowCls, lineStr) {
    var item = this.markerInnerDiv(rowCls)
  ...
  addTooltip(item, hash) {
    ...
    if (!item.getAttribute('data-has-tooltip')) {
      ...
      getCommit(this.editor.getPath(), hash.replace(/^[\^]/, ''), (msg) => {
        ...
        var avatar = gravatar.url(msg.email, { s: 80 })
